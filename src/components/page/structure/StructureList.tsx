...
const StructureCard = ({staticImage, description, name, tourId, onHover}: StructureCardProps) => {
    return <>
        <div className='flex lg:flex-row flex-col md:gap-2 gap-0 bg-primary overflow-hidden rounded'
             onMouseEnter={onHover}
             onClick={onHover}>
            <div className='flex lg:w-[700px] min-h-[200px] max-h-[400px] w-full h-full'>
                <img src={staticImage} alt={`img-${name}`} className='object-cover lg:w-[500px] w-full'/>
            </div>
            <div className='flex flex-col w-full h-full gap-2 p-2'>
                <h3 className='text-white font-medium md:text-[20px] sm:text-[18px] xs:text-[16px] text-[15px]'>{name}</h3>
                <p className='text-secondary h-full w-full md:text-[17px] sm:text-[15px] xs:text-[13px] text-[12px]'>{description}</p>
                <div>
                    <a href={`/tour?room=${tourId}`} className='flex flex-row justify-end w-full  items-center gap-2 pr-2'>
    ...
    return <div
        ...
        {/* <div
            className="md:flex-1 rounded flex-auto
             ...
             sticky top-[100px] md:p-0 p-4 md:max-h-[89vh]
             max-h-[400px] w-full overflow-hidden z-[1]">
            <StarsCanvas amount={1000} size={"0.005"} className='p-4'/>
            <div className='absolute top-[15%]'>
                <h1 className={`text-logo-yel font-medium md:text-[70px] sm:text-[60px] xs:text-[45px] text-[35px] w-full text-center`}>{selected}</h1>
            </div>
